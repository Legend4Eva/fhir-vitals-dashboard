...
def get_fhir_client():
    """Initializes and returns the FHIR client."""
    settings = {
            ...
            if code in VITAL_SIGNS_MAP:
                ...
                if value is not None:
                    ...
                    if obs.effectiveDateTime:
                        date_time = datetime.fromisoformat(obs.effectiveDateTime.isostring.replace('Z', '+00:00'))
                    elif obs.effectivePeriod and obs.effectivePeriod.start:
                        date_time = datetime.fromisoformat(obs.effectivePeriod.start.isostring.replace('Z', '+00:00'))
                    else:
...
def display_dashboard(df):
    ...
    if "high" in thresholds:
        fig.add_hline(y=thresholds["high"], line_dash="dash", line_color="red", annotation_text=f"High: {thresholds['high']}")
    if "low" in thresholds:
        fig.add_hline(y=thresholds["low"], line_dash="dash", line_color="blue", annotation_text=f"Low: {thresholds['low']}")
...
def main():
    ...
    patient_ids = fetch_patient_ids(fhir_client)
    
    if not patient_ids:
        st.error("Could not find any patient IDs with recent vital signs data on the HAPI FHIR server.")
        st.info("The dashboard requires a patient with 'vital-signs' observations. Please try again later or check the server status.")
        return
    ...
    if selected_patient_id:
        ...
        # Fetch data
        observations = fetch_vitals_data(fhir_client, selected_patient_id)
